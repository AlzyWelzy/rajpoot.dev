import LuCode from "./icons/LuCode.svelte";
import LuGraduationCap from "./icons/LuGraduationCap.svelte";

import type { ExperienceType, ProjectType, TestimonialType } from "./types";
...
// verbatim from the official Radixlink relieving letter (static/, served at
...
// publish. src/lib/seo.ts is the single source of truth for site identity.
export const emailId = siteConfig.email;

// Contact-form field limits, shared by the client inputs (maxlength) and the
// server endpoint's validation so the two can never drift apart.
