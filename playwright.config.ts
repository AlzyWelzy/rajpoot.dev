...
// passes; the matching secret makes siteverify always return success. Used so
// E2E can exercise the real contact-form -> Turnstile -> siteverify path end
// to end without flakiness from waiting on a real challenge, and without
// needing the production widget's credentials in CI. Keep in sync with
...
export default defineConfig({
  ...
  webServer: {
    // TURNSTILE_SECRET is only needed at runtime (`pnpm start`), but the site
    // key has to be present at build time too — Next inlines NEXT_PUBLIC_*
    // values into the static output. When E2E_SKIP_BUILD=1 that build already
    // happened in CI's shared `build` job, which sets the same site key.
    command:
